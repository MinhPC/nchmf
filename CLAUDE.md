...
- **Endpoint:** `https://khituongvietnam.gov.vn/WeatherApiService/api/forecast?lat={lat}&lon={lon}`
...
- **Config flow / UI**: Settings → Devices & Services → Add Integration → Vietnam Weather (NCHMF).
  Người dùng **chọn điểm trên bản đồ** (`LocationSelector`), mặc định = toạ độ nhà HA
  (`hass.config.latitude/longitude`) + tên tuỳ chọn. Config flow gọi thử API để validate,
  lấy tên phường làm title mặc định.
...
**Nếu từng chạy bản < 2.0.0 (scrape URL):** `CONF_URL` không còn; entry cũ lưu `url` sẽ
KHÔNG nạp được (thiếu `latitude/longitude`) → **xoá entry cũ và thêm lại**. `ConfigFlow.VERSION`
đã lên 2 nhưng CHƯA có `async_migrate_entry` (entry cũ hiếm, tự thêm lại đơn giản hơn).
...
| File | Trách nhiệm |
|------|-------------|
| `__init__.py` | `async_setup_entry`/`async_unload_entry` + `NchmfCoordinator` + `async_fetch_json` + `_check_health` (repair issue). Tính `current` = điểm gần giờ hiện tại. Scan interval từ `entry.options`. |
| `config_flow.py` | `async_step_user` + `async_step_reconfigure` (bản đồ lat/lon, validate qua API) + `NchmfOptionsFlow` (đổi scan interval). |
| `parser.py` | `transform(payload)` (JSON→dict), `map_condition`, `wind_direction(deg)`, `pick_current`. **Toàn bộ logic chuẩn hoá.** Hàm THUẦN, test được ngoài HA. |
...
```python
{
  ...
  "current": {...một phần tử hourly gần giờ hiện tại...},   # {} nếu hourly rỗng
...
| Entry cũ (bản URL) không nạp | thiếu `latitude/longitude` | Xoá entry, thêm lại (mục 2) |
...
- **`async_migrate_entry`**: nếu cần giữ entry của bản URL cũ (hiện yêu cầu thêm lại tay).
...
- **v2.0.0** — **ĐỔI NGUỒN sang API JSON KTTV** (`khituongvietnam.gov.vn/WeatherApiService`),
  ...
  Chưa chạy config flow trên HA thật. ConfigFlow.VERSION=2 (entry bản cũ cần thêm lại).
