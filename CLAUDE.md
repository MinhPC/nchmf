...
| File | Trách nhiệm |
|------|-------------|
| `__init__.py` | `async_setup_entry`/`async_unload_entry` + `NchmfCoordinator(hass, entry)` + `async_fetch_raw` + `_check_health` (repair issue). Scan interval từ `entry.options`. |
| `config_flow.py` | `async_step_user` + `async_step_reconfigure` (dropdown tỉnh / dán URL, validate) + `NchmfOptionsFlow` (đổi scan interval). |
| `parser.py` | `parse_html(raw)` → dict + `wind_bearing()` + `parse_provinces(raw)` (dropdown). **Toàn bộ logic scrape.** |
| `weather.py` | Weather entity — condition (+ `clear-night` ban đêm) + current + forecast daily + `native_wind_bearing` + attr `today`/`tonight` + device_info. |
| `sensor.py` | 4 sensor: nhiệt độ/độ ẩm/gió (tên theo device_class) + past-temps (`translation_key`, `_unrecorded_attributes`). Chung device_info. |
| `diagnostics.py` | `async_get_config_entry_diagnostics` — xuất coordinator.data + entry. |
| `const.py` | `DOMAIN`, `CONF_URL/NAME/SCAN_INTERVAL`, `DEFAULT_URL`, `INDEX_URL`, `ATTRIBUTION`, `*_SCAN_INTERVAL_MINUTES`, `ISSUE_NO_DATA`, `USER_AGENT`. |
| `manifest.json` | `config_flow: true`, `requirements: ["beautifulsoup4"]`. |
| `strings.json` + `translations/` | config flow, options, entity (past_temperatures), issues (no_data) — en/vi. |
...
- v1.1.0 — **dropdown chọn tỉnh** (62 tỉnh, `parse_provinces` từ INDEX_URL; DEFAULT_URL
  đổi sang slug `hai-chau-tp-da-nang-w55` để khớp option). **clear-night** ban đêm
  (dựa `sun.sun`). **Options flow** đổi scan interval (10–180′). **Diagnostics**.
  **Repairs** `no_data` khi parse thiếu dữ liệu lõi. Attr `today`/`tonight` trên weather.
  Tên entity đa ngôn ngữ (temp/hum/wind theo device_class; past-temps `translation_key`);
  `_unrecorded_attributes` cho mảng 77 phần tử. past_temps regex chọn mảng dài nhất.
  Test live (2026-07): DEFAULT parse OK (temp/forecast 9/past 77), provinces=62.
